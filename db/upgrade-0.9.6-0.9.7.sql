...
/* Add active/closed tracking to candidate_joborder */
...
/* Add comment, rejection, and edit fields to pipeline status history */
ALTER TABLE `candidate_joborder_status_history`
ADD COLUMN `comment_text` text COLLATE utf8_unicode_ci AFTER `status_to`,
ADD COLUMN `comment_is_system` int(1) NOT NULL DEFAULT '0' AFTER `comment_text`,
ADD COLUMN `rejection_reason_other` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL AFTER `comment_is_system`,
ADD COLUMN `edited_at` datetime DEFAULT NULL AFTER `site_id`,
ADD COLUMN `edited_by` int(11) DEFAULT NULL AFTER `edited_at`,
ADD COLUMN `edit_note` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL AFTER `edited_by`;

/* Add rejection reason dictionary */
...
/* Link table for status history rejection reasons */
...
INSERT INTO `rejection_reason` (`rejection_reason_id`,`label`,`created_at`,`updated_at`) VALUES
...
/* Update the existing Hired status label */
UPDATE `candidate_joborder_status`
SET `short_description` = 'Hired'
WHERE `candidate_joborder_status_id` = 800;

/* Insert new pipeline statuses */
INSERT IGNORE INTO `candidate_joborder_status`
(`candidate_joborder_status_id`,`short_description`,`can_be_scheduled`,`triggers_email`,`is_enabled`) VALUES
