...
class AddressParser
{
    ...
    public function parse($addressBlockString, $mode)
    {
        ...
        foreach ($this->_addressBlock as $lineNumber => $line) {
            if (
                $lineNumber != $cityStateZipLineArray[0] &&
    ...
    protected static function _isCityStateZip($string)
    {
        ...
        if (
            preg_match('/^' . $cityStateZip . '$/i', $string) &&
    ...
    {
        $matchFirstName  = '[a-z.\x27-]+(?:\s+[a-z.\x27-]+)*';  /* \x27 is a single quote ('). Allow multiple words */
        $matchLastName   = '[a-z.\x27-]+';      /* \x27 is a single quote ('). */

        /* Match either Lastname, Firstname or Firstname Lastname */
        $matchFirstLast = '(?:' . $matchFirstName . '\s+)' . $matchLastName;
        $matchLastFirst = $matchLastName . '\s*,\s*' . $matchFirstName;
        ...
        if (
            preg_match('/^(?:' . $matchFirstLast . '|' . $matchLastFirst . ')$/i', $string) &&
            !preg_match('/' . $company . '/i', $string)
        ) {
            return true;
        }
    ...
    protected function _getPhoneNumbers()
    {
        ...
        if ($unknownCount == 1) {
             ...
             */
            if (
                $workPhoneRow === false && $homePhoneRow !== false &&
                $cellPhoneRow !== false
            ) {
                $numbers[] = array(
             ...
             */ else if (
                $homePhoneRow === false && $workPhoneRow !== false &&
                $cellPhoneRow !== false
            ) {
                $numbers[] = array(
                    'number' => $unknownNumbers[0],
             ...
             */ else if (
                $cellPhoneRow === false && $workPhoneRow !== false &&
                $homePhoneRow !== false
            ) {
                $numbers[] = array(
                    'number' => $unknownNumbers[0],
