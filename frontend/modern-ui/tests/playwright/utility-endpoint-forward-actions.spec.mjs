...
test.describe('Utility endpoint forward action smoke', () => {
  test.skip(baseURL === '', 'Set OPENCATS_BASE_URL to run utility endpoint forward smoke checks.');

  test('calendar.dynamicdata ui=modern mounts without a runtime boundary', async ({ context, page }) => {
    await context.setExtraHTTPHeaders(buildHeaders());
    await page.setViewportSize({ width: 1366, height: 900 });

    await page.goto(buildModernRouteURL('calendar', 'dynamicData'), {
  ...
  test('wizard.ajax_getpage ui=modern mounts without a runtime boundary', async ({ context, page }) => {
    await context.setExtraHTTPHeaders(buildHeaders());
    await page.setViewportSize({ width: 1366, height: 900 });

    await page.goto(buildModernRouteURL('wizard', 'ajax_getPage'), {
      waitUntil: 'domcontentloaded'
    });
