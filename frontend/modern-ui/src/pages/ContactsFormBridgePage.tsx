...
function resolvePageCopy(actionName: string): { title: string; subtitle: string } {
  ...
  if (normalizedAction === 'add') {
    return {
      title: 'Add Contact',
      subtitle: 'Create a contact profile in the modern shell with legacy-safe form behavior.'
    };
  }

  return {
    title: 'Edit Contact',
    subtitle: 'Update contact details using the proven legacy form inside the modern UI.'
  };
}

function ContactForwardPanel({
  legacyURL
}: {
  legacyURL: string;
}) {
  ...
  return (
    <section className="modern-compat-page modern-compat-page--forward">
      <header className="modern-compat-page__header">
        <div>
          <h2 className="modern-compat-page__title">Contact Form Redirect</h2>
          <p className="modern-compat-page__subtitle">
            The contact form continues in the legacy endpoint while the modern shell keeps the handoff visible.
          </p>
        </div>
      ...
      <section className="avel-list-panel">
        <div className={`modern-state${canContinue ? '' : ' modern-state--error'}`} aria-live="polite">
          {canContinue
            ? 'Preparing legacy contact form redirect...'
            : 'Legacy contact form URL is unavailable for this route.'}
        </div>
      </section>
    </section>
  );
...
export function ContactsFormBridgePage({ bootstrap }: Props) {
  const contentCopy = resolvePageCopy(bootstrap.targetAction);
  const listURL = `${bootstrap.indexName}?m=contacts&a=listByView&ui=modern`;
    ...
    <div className="avel-dashboard-page">
      ...
      >
        <div className="modern-dashboard avel-dashboard-shell">
          <ContactForwardPanel legacyURL={bootstrap.legacyURL} />
          <section className="avel-list-panel">
            <div className="avel-list-panel__header">
              <h2 className="avel-list-panel__title">Quick Navigation</h2>
              <p className="avel-list-panel__hint">Use these links to move between the modern shell and the contact list.</p>
            </div>
