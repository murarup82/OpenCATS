...
class KpisUI extends UserInterface
{
    ...
    private function listKpis()
    {
        ...
        $customerIDRaw = isset($_GET['customerID']) ?
            $_GET['customerID'] :
            (isset($kpiStoredPrefs['customerID']) ? $kpiStoredPrefs['customerID'] : 0);
        ...
        if ($resetKpiFilters)
        {
            ...
            $customerIDRaw = (isset($kpiDefaultPrefs['customerID']) ? $kpiDefaultPrefs['customerID'] : 0);
        ...
        if ($customerID > 0 && !isset($companyData[$customerID]))
        {
            $customerID = 0;
        }
        ...
        $jobOrders = $this->getJobOrders($db, $siteID, $openOnly, $customerID);
        ...
        $filledDisplayByCompany = $this->getCompanyFilledCounts(
            $db,
            $siteID,
            new DateTime(),
            $officialReports,
        ...
        foreach ($companyData as $company)
        {
            ...
            $filledPositions = isset($filledDisplayByCompany[$companyID]) ?
            ...
            $expectedInFullPlan = (int) $company['totalPlanOpenings'] - $filledPositionsForExpected;
            if ($expectedInFullPlan < 0)
            {
                $expectedInFullPlan = 0;
            }
            $expectedFilledLastWeek = (int) round($company['expectedFilledLastWeek']) - $filledPositionsLastWeekForExpected;
            if ($expectedFilledLastWeek < 0)
            {
                $expectedFilledLastWeek = 0;
            }
            $expectedInFullPlanLastWeek = (int) $company['totalPlanOpeningsLastWeek'] - $filledPositionsLastWeekForExpected;
        ...
        $candidateScopedJobOrderIDs = null;
        if ($customerID > 0)
        {
            $candidateScopedJobOrderIDs = $eligibleJobOrderIDs;
        }
        ...
        $candidateMetricRows[] = $this->buildCandidateMetricRow(
            'Sourced Candidates',
            $this->getSourcedCandidatesCount(
                $db,
                $siteID,
                $weekStart,
                $officialReports,
        ...
        if ($candidateSourceSnapshot['total'] > 0)
        {
            ...
            $candidateSourcePieURL = sprintf(
                ...
                rawurlencode('Candidate Source Distribution'),
                implode(',', $pieLabels),
                $pieData
            );
        }
    ...
    private function details()
    {
        ...
        if ($customerID > 0)
        {
            $customerCandidateFilter = sprintf(
                "AND EXISTS (
                    SELECT 1
                    FROM candidate_joborder AS cjo_customer
                    INNER JOIN joborder AS jo_customer
                        ON jo_customer.joborder_id = cjo_customer.joborder_id
                        AND jo_customer.site_id = cjo_customer.site_id
                    WHERE cjo_customer.site_id = %s
                    AND cjo_customer.candidate_id = candidate.candidate_id
                    AND jo_customer.company_id = %s
                )",
                $db->makeQueryInteger($siteID),
                $db->makeQueryInteger($customerID)
            );
            $customerStatusFilter = sprintf(
                "AND joborder_id IN (
                    SELECT joborder_id
                    FROM joborder
                    WHERE site_id = %s
    ...
    private function getJobOrders($db, $siteID, $openOnly, $customerID = 0)
    {
        $statusFilter = '';
        if ($openOnly)
        {
    ...
    private function getCompanyFilledCounts($db, $siteID, DateTime $asOf, $officialReports, $monitoredJobOrders, $jobOrderScope, $includeFutureStatuses = false)
    {
        ...
        $rows = $db->getAllAssoc(sprintf(
            "SELECT
                jo_final.company_id AS companyID,
                COUNT(*) AS filledCount
            FROM
                (
                    SELECT
            ...
            INNER JOIN candidate_joborder_status_history AS cjh
                ...
                AND cjh.joborder_id = last_status.joborder_id
                AND cjh.date = last_status.maxDate
            INNER JOIN joborder AS jo_final
                ON jo_final.joborder_id = last_status.joborder_id
                AND jo_final.site_id = %s
            WHERE
                cjh.site_id = %s
            AND
                cjh.status_to = %s
        ...
        $countsByCompany = array();
        foreach ($rows as $row)
        {
            $countsByCompany[(int) $row['companyID']] = (int) $row['filledCount'];
        }
    ...
    private function getCandidateSourceCounts(
        $db,
        $siteID,
        DateTime $start,
        DateTime $end,
    ...
    )
    {
        ...
        if (is_array($jobOrderIDsFilter) && empty($jobOrderIDsFilter))
        {
            return array();
        }
        ...
        if (is_array($jobOrderIDsFilter))
        {
            $joinFilter .= sprintf(
                "AND EXISTS (
                    SELECT 1
                    FROM candidate_joborder AS cjo_filter
                    WHERE cjo_filter.candidate_id = candidate.candidate_id
                    AND cjo_filter.site_id = %s
                    AND cjo_filter.joborder_id IN (%s)
                )",
                $db->makeQueryInteger($siteID),
                $this->formatIntegerList($jobOrderIDsFilter)
            );
        }
        $sourceScopeFilter = $this->buildCandidateSourceScopeFilter($db, $candidateSourceScope, 'candidate');
    ...
    private function getCandidateTotalCount(
        $db,
        $siteID,
        DateTime $start,
        DateTime $end,
    ...
    private function getSourcedCandidatesCount(
        $db,
        $siteID,
        DateTime $weekStart,
        $officialReports = false,
    ...
    {
        if (is_array($jobOrderIDsFilter) || $officialReports || $candidateSourceScope !== 'all')
        {
            if ($officialReports && empty($monitoredJobOrders))
            {
                return 0;
            ...
            $row = $db->getAssoc(sprintf(
                "SELECT
                    COUNT(DISTINCT candidate.candidate_id) AS sourcedCount
                FROM
                    candidate
                WHERE
                    candidate.site_id = %s
                ...
                AND
                    LOWER(TRIM(candidate.source)) <> '(none)'
                %s
                %s",
                $db->makeQueryInteger($siteID),
                $db->makeQueryString($weekStartDate->format('Y-m-d H:i:s')),
                $db->makeQueryString($weekEndDate->format('Y-m-d H:i:s')),
                $joinFilter,
                $sourceScopeFilter
            ));
    ...
    private function getCandidateStatusCounts(
        $db,
        $siteID,
        DateTime $start,
        DateTime $end,
    ...
    private function getCandidateTrendData(
        $db,
        $siteID,
        DateTime $start,
        DateTime $end,
