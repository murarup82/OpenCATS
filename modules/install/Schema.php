    ...
    public static function get()
    {
        ...
        return array(
            ...
            '199' => '
             ...
             ',
            // middle_name column removed: not adding during install/upgrade
            ...
            ',
            '367' => 'PHP:
                $tables = $db->getAllAssoc("SHOW TABLES LIKE \'candidate_gdpr_requests\'");
                if (count($tables) == 0)
                {
                    $db->query("CREATE TABLE candidate_gdpr_requests (
                ...
                if (!isset($columnNames["gdpr_signed"]))
                {
                    $db->query("ALTER TABLE candidate ADD COLUMN gdpr_signed TINYINT(1) NOT NULL DEFAULT 0");
                }

                if (!isset($columnNames["gdpr_expiration_date"]))
                {
                    $db->query("ALTER TABLE candidate ADD COLUMN gdpr_expiration_date DATE DEFAULT NULL");
                }

                $indexes = $db->getAllAssoc("SHOW INDEX FROM candidate_gdpr_requests");
                $indexNames = array();
                foreach ($indexes as $index)
                {
                    $indexNames[$index["Key_name"]] = true;
                ...
                if (!isset($indexNames["IDX_gdpr_token_hash"]))
                {
                    $db->query("CREATE UNIQUE INDEX IDX_gdpr_token_hash ON candidate_gdpr_requests (token_hash)");
                }

                if (!isset($indexNames["IDX_gdpr_status_expires"]))
                {
                    $db->query("CREATE INDEX IDX_gdpr_status_expires ON candidate_gdpr_requests (site_id, status, expires_at)");
                }

                if (!isset($indexNames["IDX_gdpr_latest"]))
                {
                    $db->query("CREATE INDEX IDX_gdpr_latest ON candidate_gdpr_requests (site_id, candidate_id, request_id)");
                }
            ',
            '368' => 'PHP:
            ...
            ',            '370' => 'PHP:
                ...
                $sql = sprintf(
                    "INSERT INTO gdpr_notice (site_id, lang, title, body_text, updated_at)
                     SELECT s.site_id, \'ro\', \'Consimtamant GDPR - Avel Technologies\', %s, NOW()
                     FROM site s
                     WHERE NOT EXISTS (
            ...
            ',            '371' => 'PHP:
                ...
                $sql = sprintf(
                    "UPDATE gdpr_notice
                     SET
                        title = \'Consimtamant GDPR - Avel Technologies\',
                        body_text = %s
                     WHERE lang = \'ro\'",
                    $db->makeQueryString($noticeRo)
                );
            ...
            '372' => 'PHP:
                ...
                $sql = sprintf(
                    "UPDATE gdpr_notice
                     SET
                        title = \'GDPR Consent - Avel Technologies\',
                        body_text = %s
                     WHERE lang = \'en\'",
                    $db->makeQueryString($noticeEn)
                );
                ...
                $sql = sprintf(
                    "UPDATE gdpr_notice
                     SET
                        title = \'Consentement RGPD - Avel Technologies\',
                        body_text = %s
                     WHERE lang = \'fr\'",
                    $db->makeQueryString($noticeFr)
                );
            ...
            ',
            '373' => 'PHP:
                $columns = $db->getAllAssoc("SHOW COLUMNS FROM candidate_gdpr_requests");
                $columnNames = array();
                foreach ($columns as $column)
                {
                    $columnNames[$column["Field"]] = true;
                ...
                if (!isset($columnNames["accepted_lang"]))
                {
                    $db->query("ALTER TABLE candidate_gdpr_requests ADD COLUMN accepted_lang VARCHAR(5) DEFAULT NULL");
                }

                if (!isset($columnNames["notice_version"]))
                {
                    $db->query("ALTER TABLE candidate_gdpr_requests ADD COLUMN notice_version CHAR(64) DEFAULT NULL");
                }

                if (!isset($columnNames["email_to_hash"]))
                {
